...
# --- База даних (З авто-виправленням ролей) ---
def init_db():
    try:
...
# ФУНКЦІЯ ДЛЯ ПЕРЕНЕСЕННЯ КЕРІВНИКІВ
def migrate_managers():
    ...
    try:
        ...
        conn.commit(); cur.close(); conn.close()
        logging.info("✅ Керівники успішно оновлені в базі")
...
# --- Дні Народження (З правильним сортуванням від січня) ---
...
async def e_list(c: types.CallbackQuery):
    conn = psycopg2.connect(DATABASE_URL); cur = conn.cursor()
    # Сортування: Спочатку Керівники, потім Працівники, і всередині груп - за календарем (січень-грудень)
    cur.execute("""
        SELECT full_name, birth_date, role FROM employees 
        ORDER BY CASE WHEN role='Керівник' THEN 1 ELSE 2 END, 
        to_char(birth_date, 'MM') ASC, 
        to_char(birth_date, 'DD') ASC
    """)
    rows = cur.fetchall(); cur.close(); conn.close()
    res = {"Керівники": [], "Працівники": []}
    for n, d, r in rows:
        formatted = f"{d.strftime('%d.%m')} — {n}"
        res[r if r in res else "Працівники"].append(formatted)
    
    txt = "📜 **СПИСОК:**\n\n⭐ **КЕРІВНИКИ:**\n" + ("-" if not res["Керівники"] else "\n".join(res["Керівники"]))
    txt += "\n\n👥 **ПРАЦІВНИКИ:**\n" + ("-" if not res["Працівники"] else "\n".join(res["Працівники"]))
    await c.message.answer(txt, parse_mode="Markdown"); await c.answer()

# [Решта хендлерів для додавання, маршрутів та задач залишаються без змін]
# (Я пропускаю їх тут для економії місця, але вони мають бути у вашому файлі)

@dp.message(Command("start"))
async def start(m: types.Message):
    init_db()
    migrate_managers() # Запускаємо оновлення ролей при старті
    conn = psycopg2.connect(DATABASE_URL); cur = conn.cursor()
    cur.execute("INSERT INTO users (user_id, username) VALUES (%s, %s) ON CONFLICT DO NOTHING", (m.from_user.id, m.from_user.username))
    conn.commit(); cur.close(); conn.close()
    await m.answer("👋 Бот активовано! Керівників оновлено.", reply_markup=main_menu())

# Додайте сюди всі інші хендлери з попередньої версії (r_add, e_add, tgl тощо)
